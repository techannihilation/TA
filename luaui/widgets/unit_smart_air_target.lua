...
local ROTATION_PERIOD = 2                    -- Seconds required for one full rotation of the visual mark
...
local dragging = false           -- For drag-and-drop GUI
local dragOffsetX, dragOffsetY = 0, 0
local vsx, vsy = 0, 0            -- Cached screen dimensions for clamping
...
-- === UNITDEF & WEAPONDEF CACHE INITIALIZATION ===

-- Identify allowed AA units and cache relevant stats
for _, ud in pairs(UnitDefs) do
    ...
    -- Only non-commander, ground-based, anti-air units
...
-- Cache "power" (threat level) for all bombers and special bombers
...
local function SetSplitTargetState(shouldEnable, fromExternal)
...
-- Remove destroyed unit from defender list and targeting state
function widget:UnitDestroyed(unitID)
    myDefenders[unitID] = nil
    targetData[unitID] = nil
end

-- Remove unit from defenders if it is taken by another team
function widget:UnitTaken(unitID)
    myDefenders[unitID] = nil
end

-- Add new unit to defender list if it qualifies
...
-- Add unit given to player to defender list if it qualifies
...
function widget:SetConfigData(data)
    ...
    if data.splitTargetEnabled ~= nil then
        ...
        if widgetInitComplete then
            ...
            splitTargetEnabled = desiredSplitTargetState
        end
    end
    ClampGuiToScreen()
end
