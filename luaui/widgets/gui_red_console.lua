...
if Spring.GetConfigInt("Headless", 0) ~= 0 then return false end

function widget:GetInfo()
	return {
		name = "Red Console2", --version 4.1
...
local function createconsole(r)
	...
	local activationarea = {
		"area", px = r.px - r.fadedistance,
		py = r.py - r.fadedistance,
		sx = r.sx + r.fadedistance * 2,
		sy = 0,
	...
	local background = {
		...
		py = r.py,
		sx = r.sx,
		sy = r.maxlines * r.fontsize + r.margin * 2,
		color = r.cbackground,
		border = r.cborder,
...
-- search prevline and find the final instance of a colour code
...
local function clipLine(line, fontsize, maxwidth)
	...
	while 1 do
		...
		-- loop through potential positions where we might need to clip
		while 1 do
...
local function clipHistory(g, oneline)
	...
	if oneline then
		...
		if line == nil or line[1] == nil then return end
...
local function processLine(line, g, cfg, newlinecolor)
	...
	if not newlinecolor then
		...
		elseif ssub(line, 1, 1) == ">" then
			...
			--player speaking in battleroom
			if ssub(line, 1, 3) == "> <" then
	...
	if sfind(line, "~") or sfind(line, "::") then
		ignoreThisMessage = true
	end
	if sfind(line, "server=") or sfind(line, "client=") or sfind(line, "GameID:") then
		ignoreThisMessage = true
	end
...
local function updateconsole(g, cfg)
	...
	if forceupdate and (not justforcedupdate) then
		...
		if (g.vars.nextupdate < 0) or (clock() < g.vars.nextupdate) then return end
	end
...
function widget:AddConsoleLine(lines, priority)
	...
	for line in lines:gmatch("[^\n]+") do
		local processedLine = processLine(line, console, Config.console, textcolor)

		if processedLine then
			textcolor = processedLine[4]
		end
	end
