...
-- dont local it
local function MouseCursorEvent(playerID, x, z, click)
    if not isReplay and myPlayerID == playerID then return true end
    local playerPosList = playerPos[playerID] or {}

    playerPosList[#playerPosList + 1] = {
        x = x,
        z = z,
        click = click
    }
    ...
    if #playerPosList < numMousePos then return end
    playerPos[playerID] = {}
...
local function DrawCursor(playerID, wx, wy, wz, camX, camY, camZ, opacity)
    if not spIsSphereInView(wx, wy, wz, usedCursorSize) then return end
    --calc scale
