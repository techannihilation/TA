...
do --save a ton of locals
	...
	uniformBins                   = {
		...
		-- 	OPTION_TREEWIND + OPTION_SHIFT_RGBHSV,                                                                                                        -- + OPTION_VERTEX_AO
		-- 	baseVertexDisplacement = 0.0,
...
local function appendShaderDefinitionsToTemplate(template, alldefinitions)
	...
	for i, singleShaderDefs in ipairs({ "shaderDefinitions", "deferredDefinitions", "shadowDefinitions",
		"reflectionDefinitions" }) do
...
local function initMaterials()
	defaultMaterialTemplate = VFS.Include("modelmaterials_gl4/defaultMaterialTemplate.lua")
	if itsXmas then
...
local function compileMaterialShader(template, name)
	...
	local forwardShader = CompileLuaShader(template.shader, template.shaderDefinitions, template.shaderPlugins,
		name .. "_forward")
	local shadowShader = CompileLuaShader(template.shadow, template.shadowDefinitions, template.shaderPlugins,
		name .. "_shadow")
	local deferredShader = CompileLuaShader(template.deferred, template.deferredDefinitions, template.shaderPlugins,
		name .. "_deferred")
	local reflectionShader = CompileLuaShader(template.reflection, template.reflectionDefinitions, template
	.shaderPlugins, name .. "_reflection")
...
local function initBinsAndTextures()
	...
	for unitDefID, unitDef in pairs(UnitDefs) do
		if unitDef.model then
			...
			if not existingfilecache[tex1] and VFS.FileExists(tex1) then
				existingfilecache[tex1] = true
			end
			...
			if unitDef.customParams and unitDef.customParams.wrecktex1 then
				tex = unittexttures .. unitDef.customParams.wrecktex1
			else
				tex = tex1NoExt .. "_wreck.dds"
			end
			if tex ~= "" then
			...
			if not existingfilecache[tex2] and VFS.FileExists(tex1) then
				existingfilecache[tex2] = true
			end
			...
			if unitDef.customParams and unitDef.customParams.wrecktex2 then
				tex = unittexttures .. unitDef.customParams.wrecktex2
			else
				tex = tex2NoExt .. "_wreck.dds"
			end
			if tex ~= "" then
				if existingfilecache[tex] then
					textureTable[4] = tex
			...
			if existingfilecache[normalTexWreck] then
				textureTable[5] = normalTexWreck
			elseif VFS.FileExists(normalTexWreck) then
				textureTable[5] = normalTexWreck
				existingfilecache[normalTexWreck] = true
			end

			local faction = "arm"
			if unitDef.customParams and unitDef.customParams.faction then
				faction = string.lower(unitDef.customParams.faction)
			end

			if textureTable[3] ~= normalTex and textureTable[4] ~= normalTex and textureTable[5] ~= normalTex then
				local obj_uniform_bin = string.format("%s_unit_health", faction)
				if uniformBins[obj_uniform_bin] then
					objectDefToUniformBin[unitDefID] = obj_uniform_bin
				else
					objectDefToUniformBin[unitDefID] = "arm_unit_health"
				end
			else
				local obj_uniform_bin = string.format("%s_unit", faction)
				if uniformBins[obj_uniform_bin] then
					objectDefToUniformBin[unitDefID] = obj_uniform_bin
				else
					objectDefToUniformBin[unitDefID] = "arm_unit"
				end
			end
...
local function PreloadTextures()
	...
	for tex, _ in ipairs(existingfilecache) do
		Spring.Echo("[CUS GL4] Caching Texture: " .. tex)
		gl.Texture(0, tex)
	end
	gl.Texture(0, false)
...
local function AsssignObjectToBin(objectID, objectDefID, flag, shader, textures, texKey, uniformBinID, calledfrom)
	...
	if (texKey == nil or uniformBinID == nil) then
		if badassigns[objectID] == nil then
			Spring.Echo("[CUS GL4]Failure to assign to ", objectID, objectDefID, flag, shader, textures, texKey,
				uniformBinID, calledfrom)
	...
	if unitDrawBinsFlagShaderUniforms[texKey] == nil then
		...
		if flag == 1 then
			...
			if deferredrawBin[shader][uniformBinID] == nil then deferredrawBin[shader][uniformBinID] =
				unitDrawBinsFlagShader[uniformBinID] end
		end
		if debugmode then Spring.Echo("Init of bin", shader, flag, texKey, uniformBinID, "took",
				Spring.DiffTimers(Spring.GetTimerMicros(), t0, nil), "ms") end
	end
	...
	if unitDrawBinsFlagShaderUniformsTexKey.objectsIndex[objectID] then
		...
		if debugmode then Spring.Echo("AsssignObjectToBin success:", objectID, objectDefID, flag, shader, texKey,
				uniformBinID) end
	end
...
local function AddObject(objectID, drawFlag, reason)
	...
	if objectID >= 0 then
		...
		if Spring.ValidFeatureID(-1 * objectID) == false then Spring.Echo("Invalid feature for drawmask", objectID,
				objectDefID) end
		...
		if unitDrawBins[flag][shader][uniformBinID] then
			if unitDrawBins[flag][shader][uniformBinID][texKey] then
				...
				if objectIndex == numobjects then                          -- last element
					...
					if debugmode and flag == 0 then Spring.Echo("Moving", objectIDatEnd, "from", numobjects, " to",
							objectIndex, "while removing", objectID) end
					unitDrawBinsFlagShaderTexKey.objectsIndex[objectID] = nil -- pop back
					unitDrawBinsFlagShaderTexKey.objectsIndex[objectIDatEnd] =
					objectIndex                                             -- bring the last objectID to to this one
					if objectID >= 0 then                                   -- unit
						if Spring.ValidUnitID(objectIDatEnd) == true and Spring.GetUnitIsDead(objectIDatEnd) ~= true then
							unitDrawBinsFlagShaderTexKey.IBO:InstanceDataFromUnitIDs(objectIDatEnd, objectTypeAttribID,
								objectIndex - 1)
						else
							if debugmode then Spring.Echo("Tried to remove invalid unitID", objectIDatEnd,
									"while removing", objectID) end
						end
					else -- feauture
						if Spring.ValidFeatureID(-objectIDatEnd) == true then
							unitDrawBinsFlagShaderTexKey.IBO:InstanceDataFromFeatureIDs(-1 * objectIDatEnd,
								objectTypeAttribID, objectIndex - 1)
						else
							if debugmode then Spring.Echo("Tried to remove invalid featureID", -objectIDatEnd,
									"while removing", -objectID) end
						end
					end
					unitDrawBinsFlagShaderTexKey.objectsArray[numobjects] = nil -- pop back
					unitDrawBinsFlagShaderTexKey.objectsArray[objectIndex] =
					objectIDatEnd                                           -- Bring the last objectID here
					unitDrawBinsFlagShaderTexKey.numobjects = numobjects - 1
				end
			else
			...
			if debugmode then Spring.Echo("Failed to find uniformBinID for", objectID, objectDefID, texKey, shader, flag,
					uniformBinID) end
		end
	else
		if debugmode then
						...
						if type(minibinvalue) == "table" then
							...
							if minibinattr == "objectsIndex" then
								for k, v in pairs(minibinvalue) do
									local objdefname = (k >= 0 and Spring.GetUnitDefID(k) and UnitDefs[Spring.GetUnitDefID(k)].name) or
									(Spring.GetFeatureDefID(-1 * k) and FeatureDefs[Spring.GetFeatureDefID(-1 * k)].name) or
									"???"
									Spring.Echo(string.format("          %i = %i, --(%s)", k, v, objdefname))
								end
							elseif minibinattr == "objectsArray" then
								for k, v in pairs(minibinvalue) do
									local objdefname = (v >= 0 and Spring.GetUnitDefID(v) and UnitDefs[Spring.GetUnitDefID(v)].name) or
									(Spring.GetFeatureDefID(-1 * v) and FeatureDefs[Spring.GetFeatureDefID(-1 * v)].name) or
									"???"
									Spring.Echo(string.format("          %i = %i, --(%s)", k, v, objdefname))
								end
							else
...
function gadget:DrawWorldPreUnit()
	...
	if updateframe == 0 then
		...
		if (deltat > 2) and perfdebug then
			...
			Spring.Echo("[CUS GL4] [", Spring.GetDrawFrame(), "]", totalobjects, " Update time 2 < ", deltat,
				string.format("ms, per object change: %.2fus ", usecperobjectchange), totalobjects, 'objs')
			-- PERF CONCULUSION:
