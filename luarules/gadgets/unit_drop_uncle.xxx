...
-------------------------------------------------------------------------------------
-------------------------------------------------------------------------------------
local dropperID = UnitDefNames['core_uncle'].id
local unitsToDrop = UnitDefNames['corpyro'].id
local holding = {}
local holdingID = {}
local isloadedID = {}
-------------------------------------------------------------------------------------
  ...
  if (unitDefID == dropperID) and holding[unitID] then
    ...
    if currentHP <= (math.random(maxHP*.15,maxHP*.75)) then
      ...
      for i = 1, #passangers do
        ...
        Spring.SetUnitStealth(passIDs, true)
        Spring.SetUnitSonarStealth(passIDs, true)
