...
function gadget:UnitGiven(unitID, _, newTeamID, oldTeamID)
...
function gadget:AllowCommand(unitID, unitDefID, teamID, cmdID, cmdParams, _, _, _)
...
function GetUpdateInterval(teamID)
    local maxInterval = 1
    for _,resName in pairs(resTable) do
        local _, stor, _, inc, _, _, _, _, _ = spGetTeamResources(teamID, resName)
...
function UpdatePassiveBuilders(teamID, _)
    ...
    teamStalling[teamID] = {}
    for _,resName in pairs(resTable) do
        local cur, stor, _, inc, _, share, sent, rec, _ = spGetTeamResources(teamID, resName)
