...
-- We also pick one passive con, per build target, and allow it a MINOR build speed for 1 frame per interval, to prevent nanoframes that only have passive cons
...
function UpdatePassiveBuilders(teamID, interval)
    ...
    for _,resName in pairs(resTable) do
        ...
        teamStalling[teamID][resName] = cur - max(inc*stallMarginInc,stor*stallMarginSto) - 1 + (interval)*(inc-reservedExpense+rec-sent)/simSpeed --amount of res available to assign to passive builders (in next interval); leave a minor bit left over to avoid engines own "stall mode"
    ...
    for builderID in pairs(passiveCons[teamID] or {}) do
        ...
        --override buildTargetOwners build speeds for a single frame; let them build at aMINORrate to prevent nanoframes from possibly decaying
