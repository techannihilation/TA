function gadget:GetInfo()
	return {
		...
		desc    =
		"Adjusts collision volume for pop-up style units & Reduces the diameter of default sphere collision volume for 3DO models",
...
local unitCollisionVolume, pieceCollisionVolume, dynamicPieceCollisionVolume, isTorp = include(
...
if (gadgetHandler:IsSyncedCode()) then
	...
	function gadget:Initialize()
		...
		if VFS.FileExists(mapConfig) then
			...
			for _, featID in pairs(Spring.GetAllFeatures()) do
				...
				local xs, ys, zs, xo, yo, zo, vtype, htype, axis, _ = spGetFeatureCollisionData(featID)
				if (vtype >= 3 and xs == ys and ys == zs) then
					spSetFeatureCollisionData(featID, xs * rs, ys * hs, zs * rs, xo, yo - ys * 0.1323529 * rs, zo,
						vtype, htype, axis)
				end
	...
	function gadget:UnitCreated(unitID, unitDefID, unitTeam)
		...
		if bp and (bp < BP_SIZE_LIMIT) then
			...
			if pieceCollisionVolume[unitDefID] or dynamicPieceCollisionVolume[unitDefID] then
				spSetPieceCollisionData(unitID, trunkIndex, true, perPieceTrunk[1], ys, perPieceTrunk[3],
					perPieceTrunk[4], yo, perPieceTrunk[6], perPieceTrunk[7], perPieceTrunk[8])
	...
	function gadget:FeatureCreated(featureID, allyTeam)
		...
		if mapFeatures[featureModelTrim] then    -- it just might happen that some map features can have corpses
			...
			if (vtype >= 3 and xs == ys and ys == zs) then
				spSetFeatureCollisionData(featureID, xs * rs, ys * hs, zs * rs, xo, yo - ys * 0.08, zo, vtype, htype,
					axis)
			end
			...
			if defs.state ~= stateInt then
				if defs.perPiece then
					...
					for pieceIndex = 1, defs.numPieces do
						...
						if p then
							spSetPieceCollisionData(unitID, pieceIndex, true, p[1], p[2], p[3], p[4], p[5], p[6], p[7],
								p[8])
						else
							spSetPieceCollisionData(unitID, pieceIndex, false, 1, 1, 1, 0, 0, 0, 1, 1)
						end
