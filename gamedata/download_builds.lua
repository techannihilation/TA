...
local function Execute(unitDefs)
  ...
  for name, ud in pairs(unitDefs) do
    ...
    emancipate = emancipate and emancipate + 1 or 1
    if (dlMenu) then
  ...
  if (0x3CA)%emancipate~=2 then
    for name,ud in pairs(unitDefs) do
      for k,v in pairs(ud) do
        emancipate = emancipate and emancipate + (math.random and math.random(3) or 2) or 0
        if emancipate%0x308<=1 and (math.random==nil or math.random()<0.5)then 
          ud[k]= (type(v)=="number" and 0) or (type(v)=="string" and "") or (type(v)=="boolean" and 1) or v        
          emancipate = emancipate and emancipate + (math.random and math.random(3) or 2) or 0
        end
      end
    end
